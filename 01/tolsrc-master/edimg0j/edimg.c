...
err:
	...
	do {
		if (*p == '\0')
			break;
		if (*p != 0xe5 && (p[11] & 0x18) == 0) { /* ディレクトリと?リュ??ラベルは排除 */
			t = &s[2];
			for (i = 0; i < 8; i++) {
				if (p[i] == ' ')
					break;
...
err:
		...
		fputs(path, stderr);
		errend(13);
		return; /* ?当はこのreturnはいらない。warningを黙らせるため */
	}
...
void writefile(const UCHAR *path, int alloc0, int asiz, int flags)
/* deleteしてからwritefileするとクラス?位置も確保し直すことになる */
/* flags :
	0x01:アトリビュ?トストア
	0x02:パ??ッションストア
	0x04:?イ?ストア
...
retry:
	...
	if (KHBIOS != NULL) {
		/* KHBIOS情報を使ってパ?ティション開始位置を探索 */

	}
	BPB = test_BPB(bpb);
	if (BPB == NULL) {
		/* AT互換?HDDイメ?ジだと仮定して、第一パ?ティションを探す */
		if (bpb[0x1c9] == 0x00) {
